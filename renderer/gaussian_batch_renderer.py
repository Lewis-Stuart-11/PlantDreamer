...
from ..geometry.depth_utils import normalise_depth_maps
...
class GaussianBatchRenderer:
    def batch_forward(self, batch):
        ...
        for batch_idx in range(bs):
            ...
            with autocast(enabled=False):
                ...
                if render_pkg.__contains__("depth"):
                    ...
                    # Clean Depth Map
                    radius = torch.norm(batch["c2w"][batch_idx])
                    clean_depth = normalise_depth_maps(render_pkg["depth"], radius=radius)

                    depths.append(clean_depth)

                # The depth map of the original scene geometry
                if render_pkg.__contains__("original_depth"):
                    
                    # Clean Depth Map
                    radius = torch.norm(batch["c2w"][batch_idx])
                    clean_depth = normalise_depth_maps(render_pkg["original_depth"], radius=radius)

                    original_depths.append(clean_depth)
        ...
        if len(original_depths) > 0:
            outputs.update(
                {
                    "comp_original_depth": torch.stack(original_depths, dim=0).permute(0, 2, 3, 1),
                }
            )
        if len(masks) > 0:
            outputs.update(
