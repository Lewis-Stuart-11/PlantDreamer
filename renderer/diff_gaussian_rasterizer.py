...
@threestudio.register("plantdreamer-rasterizer")
class DiffGaussian(Rasterizer, GaussianBatchRenderer):
    ...
    def configure(
        ...
        self.original_geometry = copy.deepcopy(self.geometry)
    ...
    def forward(
        ...
        **kwargs
        ...
        pc = self.geometry
        pc_original = self.original_geometry
        ...
        if self.cfg.use_original_depth:
            original_screenspace_points = (
                torch.zeros_like(
                    pc_original.get_xyz, dtype=pc_original.get_xyz.dtype, requires_grad=True, device="cuda"
                )
                + 0
            )
            try:
                original_screenspace_points.retain_grad()
            except:
                pass

            # Retrieve the depth map from the original geometry 
            rendered_original_image, _, rendered_original_depth, rendered_original_alpha = rasterizer(
                means3D=pc_original.get_xyz,
                means2D=original_screenspace_points,
                shs=pc_original.get_features if override_color is None else None,
                colors_precomp=override_color if override_color is not None else None,
                opacities=pc_original.get_opacity,
                scales=pc_original.get_scaling,
                rotations=pc_original.get_rotation,
                cov3D_precomp=None,
            )
        ...
        if self.cfg.use_original_depth:
            rendered_original_depth[~mask] = rendered_original_depth[~mask].detach()
        ...
        if self.cfg.use_original_depth:
            gaussian_data["original_depth"] = rendered_original_depth
