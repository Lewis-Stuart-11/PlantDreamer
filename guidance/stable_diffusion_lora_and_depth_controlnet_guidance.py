...
from diffusers import DDIMScheduler, DDPMScheduler, StableDiffusionControlNetImg2ImgPipeline, ControlNetModel
...
@threestudio.register("stable-diffusion-lora-and-depth-controlnet-guidance")
class StableDiffusionLoraDepthControlnetGuidance(BaseObject):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        self.controlnet = ControlNetModel.from_pretrained("thibaud/controlnet-sd21-depth-diffusers", torch_dtype=torch.float32)

        # If controlnet is set, then set the strength to 1, otherwise deactivate by setting the strength to 0
        if self.cfg.use_controlnet:
            self.controlnet_conditioning_scale = 1.0
        else:
            self.controlnet_conditioning_scale = 0.0
    ...
    def set_lora(self, lora_name):
        print("Adding LoRA to stable diffusion model")

        full_lora_path = os.path.join(self.cfg.lora_path, lora_name)

        self.pipe.load_lora_weights(full_lora_path, weight_name="pytorch_lora_weights.safetensors")

        self.create_model()

    def create_model(self):
        # Configure Vae and Diffusion unet
