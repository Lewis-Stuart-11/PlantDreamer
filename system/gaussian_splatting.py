...
@threestudio.register("plantdreamer-system")
class GaussianSplatting(BaseLift3DSystem):
    ...
    def configure(self) -> None:
        ...
        # Get the prompt from plant type
        self.cfg.prompt_processor.prompt = self.geometry.plant_type.prompt

        self.guidance = threestudio.find(self.cfg.guidance_type)(self.cfg.guidance)

        # Add the LoRA if this is set in the config files
        if self.guidance.cfg.use_lora:
            self.guidance.set_lora(self.geometry.plant_type.lora_name)
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.geometry.cull_large_gaussians(self.global_step)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if "comp_original_depth" in out.keys():
            guidance_depth = out["comp_original_depth"] 
        else:
            guidance_depth = out["comp_depth"]
            
        # import pdb; pdb.set_trace()
        viewspace_point_tensor = out["viewspace_points"]

        guidance_out = self.guidance(
            guidance_rgb, guidance_depth, self.prompt_utils, **batch, rgb_as_latents=False
        )
        ...
        self.save_image_grid(
            ...
            + (
                [
                    {
                        "type": "grayscale",
                        "img": out["comp_depth"][0] if self.geometry.cfg.use_original_depth else out["comp_depth"][0],
                        "kwargs": {}
                    }
                ]
    ...
    def test_step(self, batch, batch_idx):
        ...
        if batch["index"][0] == 0:
            ...
            self.geometry.save_and_rescale_ply(save_path)
